...
Then, we can define a function that draws the solution given $\R_0$, infectious period, initial values, and maximum time step:
<<draw.sir,tidy=FALSE>>=
...
$$
Then, we can assess the fit by plotting:
<<phila_fit>>==
