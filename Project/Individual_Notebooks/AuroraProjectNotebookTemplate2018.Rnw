...
\item We will aim to have the spatial SIR model that must be checked by Dr. Earn (part b of the synchronization question) done by 19 March
\end{itemize}
...
\item Nailed down a plan for what exactly we want to do with the numerical simulations. The new outlook of our project will be what changing parameters ($R_0$ mostly), seasonal forcing, and adding stochasticity do to the patch model.
\end{itemize}
