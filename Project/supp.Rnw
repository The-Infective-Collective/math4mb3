...
Here, we do not use random initial conditions as it may lead to exaggeration of global extinction probability.
Instead, we start from the endemic equilibrium.
Once again, this simulation takes approximately 11 hours. Thus, we load a saved file instead.
