...
for (m in mvec) {
  ...
  for (R in R0vec) {
    ...
    for (i in 1:nsim) {
      ...
      incoherence2 <- apply(df[(nrow(df) - 2*365):nrow(df), c("I1", "I2")], 1, coherence_calc_scaled) %>% mean()
      incoherence5 <- apply(df[(nrow(df) - 5*365):nrow(df), c("I1", "I2")], 1, coherence_calc_scaled) %>% mean()
...
save("reslist", file="R0_highm.rda")
