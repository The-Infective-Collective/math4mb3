...
plot(philadata$date, philadata$pim, type="n", # don't actually plot anything
     ...
     xaxt='n', #surpress x ticks and labels
     xlab="",
     ann=FALSE, # no axis annotation (i.e., no title or axis labels)
     #yaxs="i", # keep this line out because we want the small space
     xaxs="i",
...
mtext("P&I Deaths", side=2, at=900, line=-4, font=1, las=1, cex=1.75, col="blue") #line =-5 is because the y axis hits 5 letters from the end (after the D in deaths)
...
## plot data
lines(philadata, col="grey", lwd=1.75) #lwd gives line thickness relativ to default
...
lm(formula ="date~log(pim)", data= x) #generates lm
