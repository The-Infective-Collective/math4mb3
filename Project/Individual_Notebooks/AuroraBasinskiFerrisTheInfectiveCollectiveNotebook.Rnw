...
\begin{itemize}
\item Worked on developing some code which will spit out whether or not coherent based on many R_0 values and with many initial conditions. It takes a long time to run. We will each run half the simulation overnight and then reconvene tomorrow to discuss.
\end{itemize}
...
\textbf{Solo Work}
\emph{Approximate Duration: 30 mins}
\begin{itemize}
...
\emph{Group work:} 13.5 hours

\emph{Solo work:} 5.5 hours
